...
selecteddates = []
samedate = []
feildnames = ['name', 'date1', 'date2', 'site1', 'number', 'email', 'cardnumber', 'expirey', 'cardname', 'cvv', 'food']
...
def pricecheck():
    global multuply
    multuply = (calen2.get_date() - calen.get_date()).days
    if site == 'Oak':
        pass
    ...
    if site == 'Kauri':
        ...
        pricelabel.config(text='Price: $' + str(price*multuply))
            ...
            if searchvar.get() == 'Month':
                ...
                for wor in searchdates:
                    if searchbox.get() == str(wor.month):
                        o = 0
                        i = i + 35
                        label = Label(tab5, font=('', 16), text=(row.name + ' | ' + row.date1 + ' | ' + row.date2 + ' | ' + row.site1), bg='light blue')
                        labellist.append(label)
                        label.place(x=30 + ll, y=150 + i)
    ...
    with open('orders.csv', 'rt')as f:
        ...
        for row in kaurilist:
            for wor in selecteddates:
                if row == wor:
                    kauriselect.configure(bg='orangered', state='disabled')
                    if site == 'Kauri':
                        site = 'Not Chosen'

                    kaurinot.config(text='Unavailable')
                    kaurinot.place(x=755, y=20)

        for row in oaklist:
            for wor in selecteddates:
                if row == wor:
                    oakselect.configure(bg='orangered', state='disabled')
                    # oakselect.map('oakselected', foreground=[('disabled', 'black')])
                    if site == 'Oak':
                        site = 'Not Chosen'

                    oaknot.config(text='Unavailable')
                    oaknot.place(x=121, y=20)

        for row in willowlist:
            for wor in selecteddates:
                if row == wor:
                    willowselect.configure(bg='orangered', state='disabled')
                    if site == 'Willow':
                        site = 'Not Chosen'

                    willownot.config(text='Unavailable')
                    willownot.place(x=430, y=20)
...
dates = Label(tab1, text='Dates', font=('Courier', 20, 'bold'), bg='Light Blue', fg='black')
dates.place(x=450, y=150)
...
foodcheck = Checkbutton(tab4, text='Food Option', font=('Courier', 15), command=foodchange, bg='Light Blue', activebackground='light blue')
foodcheck.place(x=50, y=200)

pricelabel = Label(tab4, text='Price: $' + str(price), font=('Courier', 15), bg='lightblue')
pricelabel.place(x=50, y=250)
...
def oakfun():
    ...
    kauriselected = False
    site = 'Oak'
    if willowselect.cget('bg') != 'orangered':
        willowselect.configure(bg='Dodger Blue')
        willownot.destroy()
    if kauriselect.cget('bg') != 'orangered':
...
def willowfun():
    ...
    kauriselected = False
    site = 'Willow'
    if oakselect.cget('bg') != 'orangered':
...
kaurinot = Label(tab3, text='Unavailable', font=('Courier', 12), fg='black', bg='light blue')

oaknot = Label(tab3, text='Unavailable', font=('Courier', 12), fg='black', bg='light blue')

willownot = Label(tab3, text='Unavailable', font=('Courier', 12), fg='black', bg='light blue')
